...
renderCUDA(
	...
	const float* __restrict__ plane_d,
	...
	float* __restrict__ dL_dplane,
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dcolors,
	float* __restrict__ dL_dsemantics, 
	const bool detach_sem)
{
	...
	__shared__ float collected_sems[NUM_SEMANTICS * BLOCK_SIZE];
	__shared__ float3 collected_Tu[BLOCK_SIZE];
	__shared__ float3 collected_Tv[BLOCK_SIZE];
	__shared__ float3 collected_Tw[BLOCK_SIZE];
	__shared__ float collected_plane[BLOCK_SIZE];
	...
	if (inside) {
		...
		for (int i = 0; i < NUM_SEMANTICS; i++){
			dL_dsem[i] = dL_depths[(SEMANTIC_OFFSET + i) * H * W + pix_id];
		}

		dL_dp = dL_depths[PLANE_OFFSET * H * W + pix_id];
	...
	float accum_sems_rec[NUM_SEMANTICS] = {0};
	float accum_plane_rec = 0;
	...
	float last_sem[NUM_SEMANTICS] = {0};
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_plane[block.thread_rank()] = plane_d[coll_id];
			for(int i = 0; i < NUM_SEMANTICS; i++){
				collected_sems[i * BLOCK_SIZE + block.thread_rank()] = semantics[coll_id * NUM_SEMANTICS + i];
			}
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			accum_plane_rec = last_alpha * last_plane + (1.f - last_alpha) * accum_plane_rec;
			last_plane = collected_plane[j];
			dL_dalpha += (collected_plane[j] - accum_plane_rec) * dL_dp;
			atomicAdd(&(dL_dplane[global_id]), w * dL_dp);

			for (int i = 0; i < NUM_SEMANTICS; i++){
				const float sem = collected_sems[i * BLOCK_SIZE + j];
				accum_sems_rec[i] = last_alpha * last_sem[i] + (1.f - last_alpha) * accum_sems_rec[i];
				last_sem[i] = sem;
				if(!detach_sem){
					dL_dalpha += (sem - accum_sems_rec[i]) * dL_dsem[i];
					// float grad = (sem - accum_sems_rec[i]) * dL_dsem[i];
					// printf("alpha_grad %d: %f, ", i, grad);
					// printf("sem %d: %f, ", i, sem);
					// printf("accum_sems_rec %d: %f, ", i, accum_sems_rec[i]);
					// dL_dalpha += grad;
				}
				atomicAdd(&(dL_dsemantics[global_id * NUM_SEMANTICS + i]), w * dL_dsem[i]);
			}
	 ...
	 glm::vec4* dL_drots)
{
	...
	if (Ts_precomp != nullptr) {
		...
		normal_world = {L[2].x, L[2].y, L[2].z};
	}
	...
	float3 dL_dtn = transformVec4x3Transpose(dL_dnorm, viewmatrix);
	dL_dtn = make_float3(
		dL_dtn.x + dL_dplane[idx] * p_orig.x,
		dL_dtn.y + dL_dplane[idx] * p_orig.y,
		dL_dtn.z + dL_dplane[idx] * p_orig.z
	);
	...
	dL_dmeans[idx] = glm::vec3(dL_dM[2].x + dL_dplane[idx] * normal_world.x * multiplier, 
										 dL_dM[2].y + dL_dplane[idx] * normal_world.y * multiplier, 
										 dL_dM[2].z + dL_dplane[idx] * normal_world.z * multiplier);
}
