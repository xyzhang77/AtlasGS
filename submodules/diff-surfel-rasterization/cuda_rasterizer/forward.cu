...
__device__ void compute_transmat(
	...
	distance = p_orig.x * L[2].x + p_orig.y * L[2].y + p_orig.z * L[2].z;
	...
	bool prefiltered)
{
	...
	{
		compute_transmat(((float3*)orig_points)[idx], scales[idx], scale_modifier, rotations[idx], projmatrix, viewmatrix, W, H, T, normal, distance);
	...
	float* __restrict__ out_others)
{
	...
	// __shared__ float collected_semantic[NUM_SEMANTICS * BLOCK_SIZE];
	// __shared__ float collected_features[CHANNELS * BLOCK_SIZE];
	// __shared__ float collected_plane_d[BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			// 	collected_features[block.thread_rank() + i * BLOCK_SIZE] = features[coll_id * CHANNELS + i];
			// for (int i = 0; i < NUM_SEMANTICS; i++)
			// 	collected_semantic[block.thread_rank() + i * BLOCK_SIZE] = semantics[coll_id * NUM_SEMANTICS + i];
			// collected_plane_d[block.thread_rank()] = plane_d[coll_id];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < NUM_SEMANTICS; ch++)
				S[ch] += semantics[collected_id[j] * NUM_SEMANTICS + ch] * w;
			
			distance += plane_d[collected_id[j]] * w;
	...
	if (inside)
	{
		...
		for (int ch=0; ch<NUM_SEMANTICS; ch++){
			out_others[pix_id + (SEMANTIC_OFFSET+ch) * H * W] = S[ch];
		}
		out_others[pix_id + PLANE_OFFSET * H * W] = distance;
