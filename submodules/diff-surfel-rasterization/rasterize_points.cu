	...
	const bool debug)
{
  ...
  torch::Tensor out_others = torch::full({3+3+1+NUM_SEMANTICS+1, H, W}, 0.0, float_opts);
	...
	const bool detach_sem) 
{
  ...
  torch::Tensor dL_dsemantics = torch::zeros({P, NUM_SEMANTICS}, means3D.options());
  torch::Tensor dL_dnormal = torch::zeros({P, 3}, means3D.options());
  torch::Tensor dL_dopacity = torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_dplane = torch::zeros({P, 1}, means3D.options());
  ...
  {  
	  ...
	  dL_dplane.contiguous().data<float>(),
