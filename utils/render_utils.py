...
def rotation_angle(R1,R2):
    """Calculate the angle between two rotation matrix."""
    rot_diff = R1.inv() * R2
    rotation_degree_rad = rot_diff.magnitude()
    rotation_degree = np.degrees(rotation_degree_rad)  # Convert to degrees
    return rotation_degree

def pose_interpolation(pose_start, pose_end, num_poses=25, angle_per_frame=3):
    """
    Copy from NVS-Solver

    Args:
    pose_start: from world to camera_start
    pose_end: from world to camera_end
    """
    A = pose_start[:3, 3].reshape((1,3))
    C = pose_end[:3, 3].reshape((1,3)) 

    rotations = R.from_matrix([pose[:3, :3] for pose in [pose_start, pose_end]])
    angle_between_rotations = rotation_angle(rotations[0], rotations[1])
    distance_between_trans = np.linalg.norm(np.linalg.inv(pose_start)[:3,3] - 
                                            np.linalg.inv(pose_end)[:3,3])
    print(f"[Info] Rotation angle between matrices: {angle_between_rotations} degrees" + \
          f"Translation between matrices: {distance_between_trans}")
    if num_poses <= 0:
        num_poses = int(angle_between_rotations / angle_per_frame) + 1
        print(f"[Info] set number of poses as {num_poses}")

    # #####################################
    if num_poses <= 2:
       return np.stack([pose_start, pose_end])
    times = np.array([0, num_poses-1])
    interp_times = np.linspace(0, num_poses-1, num_poses) 
...
def generate_interp_cams(viewpoint_cameras, n_views=-1, n_frames=300):
    # Get camera-to-world transformations from viewpoint_cameras and convert to numpy
    ...
    # Convert to world-to-camera transformations
    w2cs = np.array([np.linalg.inv(np.concatenate([c2w, np.array([[0,0,0,1]])])) for c2w in pose_recenter])

    # Determine number of views for interpolation
    if n_views <= 0:
        n_views = len(w2cs)
    frames_per_segment = n_frames // (n_views-1)  # 10second, fps=30
    remainder = n_frames % (n_views - 1)

    # Interpolate between W2C poses
    all_inter_pose_w2c = []
    for i in range(n_views - 1):
        n_interp = max(2, frames_per_segment + (1 if i < remainder else 0))  # Distribute remainder
        tmp_inter_pose = pose_interpolation(w2cs[i], w2cs[i+1], n_interp)[:, :3, :]
        all_inter_pose_w2c.append(tmp_inter_pose)
    # Flatten the interpolated poses list and reshape to (n_frames, 3, 4)
    all_inter_pose_w2c = np.concatenate(all_inter_pose_w2c).reshape(-1, 3, 4)

    # Create camera list based on the interpolated W2C poses
    traj = []
    for w2c in all_inter_pose_w2c:
        # Make w2c a 4x4 matrix
        w2c = np.concatenate([w2c, np.array([[0, 0, 0, 1]])], axis=0)

        # Invert W2C back to C2W to get camera-to-world transformation
        c2w = np.linalg.inv(w2c)
        c2w = c2w @ np.diag([1, -1, -1, 1])
        w2c = np.linalg.inv(c2w)

        # Deep copy the first camera
        cam = copy.deepcopy(viewpoint_cameras[0])
        cam.image_height = int(cam.image_height / 2) * 2
        cam.image_width = int(cam.image_width / 2) * 2

        # Update camera transforms with the new W2C matrix
        cam.world_view_transform = torch.from_numpy(w2c.T).float().cuda()  # Convert to torch tensor and transpose
...
def create_videos(base_dir, input_dir, out_name, num_frames=480):
  ...
  for k in ['depth', 'normal', 'color', 'depth_normal', 'alpha', 'depth_median', 'semantic']:
    video_file = os.path.join(base_dir, f'{video_prefix}_{k}.mp4')
    input_format = 'gray' if k == 'alpha' else 'rgb'
    

    file_ext = 'png' if k in ['color', 'normal', 'depth_normal', 'semantic'] else 'tiff'
    idx = 0
    ...
    with media.VideoWriter(
      ...
      for idx in tqdm(range(num_frames)):
        ...
        if k in ['color', 'normal', 'depth_normal', 'semantic']:
          img = img / 255.
