...
def map_semantic_to_color(semantic_map):
    """_summary_

    Args:
        semantic_map (torch.Tensor): A tensor of shape (H, W) containing the semantic map.
    
    Returns:
        torch.Tensor: A tensor of shape (3, H, W) containing the color map.
    """

    num_classes = len(LABEL_MAP.keys())
    color = (num_classes - semantic_map) / num_classes
    img = colormap(color.cpu().numpy())
