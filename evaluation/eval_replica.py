...
def calc_3d_metric(rec_meshfile, gt_meshfile, out_dir = None, align=False):
    """
    3D reconstruction metric.

    """
    if out_dir is None:
        out_dir = os.path.join(os.path.dirname(rec_meshfile), "evaluation")
    ...
    metric = {
        ...
        "completion": completion_ratio_rec,
        "fscore": fscore,
        "normal_acc": normal_acc,
        "normal_comp": normal_comp,
        "normal_avg": normal_avg
    }
    with open(f'{out_dir}/metric.json', 'w') as f:
    ...
    name = os.path.basename(rec_meshfile).replace('.ply', '')
    cmap = plt.get_cmap('hot_r')
    data_color = cmap(np.minimum(dist_d2s, vis_dist) / vis_dist)[:, :3]
    write_vis_pcd(f'{out_dir}/{name}_precision.ply', rec_pc_tri.vertices, data_color)
    ...
    stl_color = cmap(np.minimum(dist_s2d, vis_dist) / vis_dist)[:, :3]
    write_vis_pcd(f'{out_dir}/{name}_recall.ply', gt_pc_tri.vertices, stl_color)
...
if __name__ == '__main__':
    ...
    calc_3d_metric(args.rec_mesh, args.gt_mesh, args.outdir)
