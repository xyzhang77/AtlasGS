...
from utils.colmap_utils import read_images_text, qvec2rotmat
...
if __name__ == "__main__":
    ...
    intrinsic_path = os.path.join(os.path.join(args.data_dir, args.scene, "intrinsic_color.txt"))
    ...
    if args.gt_mesh is not None:
        gt_mesh = os.path.join("gts", "scannet", args.scene[5:]+'.obj')
        gt_mesh = trimesh.load(gt_mesh)

    if args.result_folder is None:
        args.result_folder = os.path.join(os.path.dirname(args.mesh_path), 'evaluation')
    os.makedirs(args.result_folder, exist_ok=True)
    metrics = evaluate(mesh, gt_mesh, save_path=args.result_folder)
    print(metrics)